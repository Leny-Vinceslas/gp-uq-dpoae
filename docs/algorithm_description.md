...
L2 value where the posterior sample reaches its maximum amplitude in the **compressive region (L2 ≥ 40 dB)**. Uses the GP sample directly — no re-fitting.
...
For each remaining step i = 1 … (k-3):
    ...
    If not the last step:
        Refit GP posterior with FIXED hyperparameters (no Adam — fast)
```
