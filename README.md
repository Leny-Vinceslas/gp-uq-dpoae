...
the accompanying manuscript is still in preparation and some analyses
(e.g. proper censored-likelihood noise model) are noted as future work below.

## What this is

A Gaussian Process (GP) regression pipeline for **DPOAE input/output (I/O)
growth functions** — the curves audiologists use to characterise cochlear
compression from otoacoustic emissions. The project asks two related
questions:

1. **How few stimulus levels can we measure and still recover the I/O curve
   and its clinically relevant features (compression knee, max-compression
   level, slopes) with usable uncertainty bounds?** (`experiment.py`)
...
leave-*k*-out design compares sparse fits (*k* = 3–9 of 10 stimulus levels)
against a full reference fit, evaluating curve RMSE, feature bias, and
95% CI coverage. A separate residual-bootstrap analysis (`bootstrap_retest.py`)
estimates genuine two-session minimum detectable change (MDC) for the
extracted features, to benchmark against the single-session GP posterior SD.
...
- **Reported MDC is explicitly framed as an estimated single-session lower
  ...
  dataset. The bootstrap analysis exists precisely to sanity-check how close
  that lower bound gets to a bootstrapped two-session estimate.
...
├── figures.py                   # all publication figures (1-5)
├── plot_algorithm.py            # pipeline flowchart diagram
├── plot_curves.py               # per-participant GP fit diagnostics
├── docs/
...
This is a human-subjects study (38 normal-hearing adults); **no raw or
per-participant data is included in this repository.** `outputs/` ships
only cohort-level figures aggregated across all curves (e.g.
`fig2_rmse_vs_sparsity`, `fig3_feature_uncertainty`, `fig5_cohort_features`);
per-participant CSVs, curve-fit plots, and the single-participant example
figure (`fig1_example_curve`, which labels its panel with a real study ID)
...
# 2. Per-participant GP fit diagnostics
python plot_curves.py                     # all participants
python plot_curves.py --id <participant_id>  # single participant

# 3. Full leave-k-out experiment (parallel across curves)
...
|---|---|---|
| Kernel | Matérn-3/2 over stimulus level (L2) | Once-differentiable — matches smooth cochlear compression, not over-smooth like RBF |
...
## Current limitations (tracked, not hidden)
...
2. Active selection is constrained to the 10 pre-defined stimulus positions,
   limiting its advantage over a prospective fine-grid protocol.
...
4. No population pooling — features are estimated per curve, appropriate
   for individual (e.g. ototoxicity) monitoring but not for population-level
   inference.
...
- All parameter/behaviour changes to the analysis code are logged in
  [`CHANGELOG.md`](CHANGELOG.md) with the scientific rationale, not just
  the diff — kept up to date alongside the manuscript.

## Author

Leny Vinceslas — UCL. Part of ongoing DPOAE dosimetry / ototoxicity
monitoring research.
